...
if sudo systemctl is-active --quiet roomdisplay; then
  ...
  echo ""
  echo "  Optional — enable HTTPS (required for the 'wake on camera motion'"
  echo "  feature): bash $REPO_DIR/scripts/setup-https.sh"
  echo "  See deploy/caddy/README.md for details."
  echo ""
